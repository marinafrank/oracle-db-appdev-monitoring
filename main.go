...
var (
	...
	defaultFileMetrics = kingpin.Flag("default.metrics", "File with default metrics in a toml or yaml format. (env: DEFAULT_METRICS)").Default(getEnv("DEFAULT_METRICS", "default-metrics.toml")).String()
	customMetrics      = kingpin.Flag("custom.metrics", "Comma separated list of file(s) that contain various custom metrics in a toml or yaml format. (env: CUSTOM_METRICS)").Default(getEnv("CUSTOM_METRICS", "")).String()
